...
def new_parser():
    ...
    # print block
    print_block_parser = sub_parser.add_parser(
        'printblock', help='print a single block')
    print_block_parser.add_argument(
        '--height', type=int, dest='height', help='height of block')
...
def print_block(height):
    ...
    print("Prev. hash: {0}".format(target.prev_block_hash))
    print("Hash: {0}".format(target.hash))
    pow = Pow(target)
    print("Pow: {0}".format(pow.validate()))
